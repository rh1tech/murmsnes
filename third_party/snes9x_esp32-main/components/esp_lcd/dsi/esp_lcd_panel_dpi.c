...
#define LINE_BUFFER_HEIGHT (3)  // scaling factor
...
static bool dma_trans_done_cb(dw_gdma_channel_handle_t chan, const dw_gdma_trans_done_event_data_t *event_data, void *user_data)
{
    ...
    for (int j = 0; j < LINE_BUFFER_HEIGHT; j++) {
        if (j == LINE_BUFFER_HEIGHT - 1)
...
static esp_err_t dpi_panel_create_dma_link(esp_lcd_dpi_panel_t *dpi_panel)
{
    ...
    // fcipaq: Use the DMA to scan out each scanline LINE_BUFFER_HEIGHT times for nearest neighbor scaling
    dw_gdma_link_list_config_t link_list_config = {        
        .num_items = LINE_BUFFER_HEIGHT,  // fcipaq: from mipi_dsi_priv.h: "NOTE: we assume 1 DMA link item can carry the WHOLE image"
...
static esp_err_t dpi_panel_init(esp_lcd_panel_t *panel)
{
    ...
    for (int i = 0; i < dpi_panel->num_fbs; i++) {
        ...
        // fcipaq: program the DMA to send each framebuffer LINE_BUFFER_HEIGHT times
        for (int j = 0; j < LINE_BUFFER_HEIGHT; j++)
